    ...
    function render(opts) {
        ...
        switch (type) {
            ...
            case 'light':
                ...
                classes.header.fg = $.toastDefaults.style.light || 'text-dark';
                break;
        }
        ...
        if (dismissible) {
            html += `<button type="button" class="btn-close ms-auto ${classes.dismiss}" data-bs-dismiss="toast" aria-label="Close"></button>`;
        }

        html += `</div>`;

        if (content) {
            html += `<div class="toast-body" style="overflow-wrap: break-word">
                        ${content}
                    </div>`;
        }
        ...
        if (!$.toastDefaults.stackable) {
            ...
            toast= new bootstrap.Toast(toastContainer.find('.toast:last'))
            toast.show()
        ...
        if ($.toastDefaults.pauseDelayOnHover && opts.delay) {
            ...
            $(document).on('mouseleave', '#' + id, function ()
            {
                const
                    ...
                    future = parseInt($(this).data('bsHideAfter'));
