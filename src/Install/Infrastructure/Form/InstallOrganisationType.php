...
class InstallOrganisationType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        $countrySubscriber = new CountryFieldSubscriber($factory, ['options'=>['required' => true,'attr'=>['field_id'=>376]]]);
        $builder->addEventSubscriber($countrySubscriber);

        $admin1Subscriber = new Admin1FieldSubscriber($factory, $countrySubscriber->getCountry(), ['options'=>['attr'=>['field_id'=>377]]]);
        $builder->addEventSubscriber($admin1Subscriber);

        $admin2Subscriber = new Admin2FieldSubscriber($factory, $admin1Subscriber->getCountry(), $admin1Subscriber->getAdmin1());
        $builder->addEventSubscriber($admin2Subscriber);

        $builder->addEventSubscriber(new Admin3FieldSubscriber($factory, $admin2Subscriber->getAdmin2()));
