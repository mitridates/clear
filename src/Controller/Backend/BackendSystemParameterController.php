...
class BackendSystemParameterController extends AbstractController
{
    ...
    public function indexAction(Request $request, ParameterBagInterface $bag): Response
    {
        ...
        $form = $this->createForm(SystemParameterSearchType::class, new SystemParameter(),$locales)->handleRequest($request);
...
//    }
    ...
    public function editAction(Request $request, SystemParameter $entity, EntityManagerInterface $em, ParameterBagInterface $bag): Response
    {
        ...
        $response = $this->_updateRequest(
            ...
            ['onFlush' => function (EntityManagerInterface $em, FormInterface $form, SystemParameter $entity, Response $response) use ($bag) {
                $uow= $em->getUnitOfWork();
                $uow->computeChangeSets();
                $changeSet = $uow->getEntityChangeSet($entity);
                $hasChanges= !empty($changeSet);
                if ($hasChanges) {
                    $response->headers->add(['AACustomUpdated'=>count($changeSet)]);
