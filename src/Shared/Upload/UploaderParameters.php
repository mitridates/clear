...
class UploaderParameters
{
    ...
    public function __construct(
        string $kernel_project_dir,
        Arraypath $bundle_parameters,
        string $resource)
    {
        $config = $bundle_parameters['upload'];
        $res=& $config['resource'][$resource];
        $p= [
            'kernelProjectDir' => $kernel_project_dir,
            'absUploadDir'=>null,
            'cache'=>[
                'dir'=>'/tmp',
                'absDir'=>null,
...
# (default 'center').
        ...
        $p['absUploadDir']= self::getAbsResourceDir($kernel_project_dir, $res['uploadDir']);
        if($res['thumbDir'])
        {
