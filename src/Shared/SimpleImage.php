...
class SimpleImage
{
     ...
     */
    public function toFile($fileName, $mimeType = null, $quality = 100)
    {
        $image = $this->generate($mimeType, $quality);

        // Save the image to file
        if (!file_put_contents($fileName, $image['data'])) {
            throw new \Exception("Failed to write image to file: $fileName", self::ERR_WRITE);
        }
    ...
    public function textBox($text, $options)
    {
        ...
        if ($align <> 'justify') {
            ...
            foreach ($lines as $keyLine => $line) {
                ...
                $words[0] = SimpleImage . phpstr_repeat(' ', $spaces) . $words[0];
